...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if range_left.data > 0.2:  #too far from left
            ang_vel = (range_left.data - 0.15)
            lin_vel = 0.03



        if range_centre_left.data < 0.18:  #too close to left
            ang_vel = (-0.01/(range_centre_left.data+0.0000001))
            lin_vel = 0.03

        if range_centre_left_left.data < 0.14: 
            ang_vel = (-0.01/(range_centre_left_left.data+0.0000001))
            lin_vel = 0.04

        if range_left.data < 0.1:
            ang_vel = (-0.01/(range_left.data+0.0000001))
            lin_vel = 0.05



        if range_right.data < 0.14: #too close to right
            ang_vel = (0.01/(range_right.data+0.0000001))
            lin_vel = 0.04

        if range_centre_right.data < 0.14:
            ang_vel = (0.01/(range_centre_right.data+0.0000001))
            lin_vel = 0.03


        if range_centre.data < 0.25: #too close ahead
            lin_vel = 0.01
            ang_vel = -1
